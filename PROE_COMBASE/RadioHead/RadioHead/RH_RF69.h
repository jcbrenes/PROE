...
#define RH_RF69_MAX_MESSAGE_LEN (RH_RF69_MAX_ENCRYPTABLE_PAYLOAD_LEN - RH_RF69_HEADER_LEN) //60 bytes for message
#endif
...
//	Test,
    ...
    bool        recv(uint8_t* buf, uint8_t* len, unsigned long &timeStamp1);
    ...
    bool        send(const uint8_t* data, uint8_t len);
	    ...
	    /// Waits until any previous transmit packet is finished being transmitted with waitPacketSent().
    /// Then loads a message that contains a time stamp using the RTC value into the transmitter
    /// and starts the transmitter.
    /// \return true if the message was queued for transmit
    bool        sendTimeStamp();
