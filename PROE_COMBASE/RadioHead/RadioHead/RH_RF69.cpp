...
bool RH_RF69::recv(uint8_t* buf, uint8_t* len, unsigned long &timeStamp1)
{
    if (!available())
	return false;
...
bool RH_RF69::sendTimeStamp()
{
	...
	// Get the Leader TimeStamp. THIS HAS TO BE DONE HERE, BECAUSE IF IT'S DONE IN THE SPI.TRANSFER PROCESS, YOU'LL GET GARBAGE ON THE OTHER SIDE.
	uint8_t byte1, byte2, byte3, byte4;
	unsigned long leaderTimeStamp = RTC->MODE0.COUNT.reg;
	byte4 = leaderTimeStamp >> 24;
	byte3 = leaderTimeStamp >> 16;
	byte2 = leaderTimeStamp >> 8;
	byte1 = leaderTimeStamp;
    ATOMIC_BLOCK_START;
    digitalWrite(_slaveSelectPin, LOW);
    _spi.transfer(RH_RF69_REG_00_FIFO | RH_RF69_SPI_WRITE_MASK); // Send the start address with the write mask on
    _spi.transfer(4 + RH_RF69_HEADER_LEN); // Include length of headers PLUS 32 BITS (4 bytes) OF TIMESTAMP
    // First the 4 headers
    _spi.transfer(_txHeaderTo);
    _spi.transfer(_txHeaderFrom);
    _spi.transfer(_txHeaderId);
    _spi.transfer(_txHeaderFlags);
	// Now insert the Leader TimeStamp
	_spi.transfer(byte1);
	_spi.transfer(byte2);
	_spi.transfer(byte3);
	_spi.transfer(byte4);
