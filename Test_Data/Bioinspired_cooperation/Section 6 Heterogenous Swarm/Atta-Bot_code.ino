...
const uint8_t cantidadRobots = 8; //Cantidad de robots en enjambre. No cuenta la base, solo los que hablan.
...
const int unidadAvance= 1500; //medida en mm que avanza cada robot por movimiento
const int unidadRetroceso= -90; //medida en mm que retrocede el robot al encontrar un obstáculo
...
//Constantes para detección de feromona
const int max_delay_mag = 1000; //número max de ciclos de delay hasta que se estabilice el filtro de la coordenada Z
const float umbral_deteccion_HSC = 40.0; //valor umbral para la detección con el magnetómetro HSC
const float umbral_deteccion_MAG = 40.0; //valor umbral para la detección con el magnetómetro MAG


//VARIABLES GLOBALES

//Variables para la máquina de estados principal
enum PosiblesEstados {AVANCE=0, RETROCEDA, GIRE_DERECHA, GIRE_IZQUIERDA, ESCOGER_DIRECCION, GIRO, CONTROL_POSE, NADA, DELAY_INICIAL};
//char const *PosEstados[] = {"AVANCE", "RETROCEDA", "GIRE_DERECHA", "GIRE_IZQUIERDA","ESCOGER_DIRECCION","GIRO", "NADA"};
PosiblesEstados estado = DELAY_INICIAL;
...
//VARIABLES FEROMONA
#define HSCDTD008A_ADDRESS 0x0C
...
//VARIABLES PARA DETECCIÓN DE FEROMONA
int contador_delay_mag = 0;
...
void setup() {
  ...
  //Inicializacion de la brújula digital
  inicializaMagnet();
  resetVarGiro();
  
  //Inicialización de los magnetómetros
  iniciar_HSCDTD008A(); 
  iniciar_MAG3110();
...
void loop(){
  ...
  //POLLING MAGNETÓMETROS
  leer_HSCDTD008A(hsc_x, hsc_y, hsc_z);
  ...
  if (contador_delay_mag > max_delay_mag){
    if (abs(HSC_Z_Filtrado) > umbral_deteccion_HSC && !deteccion_HSC){
      deteccion_HSC = true;
    }
    if (abs(MAG_Z_Filtrado) > umbral_deteccion_MAG && !deteccion_MAG){
      deteccion_MAG = true;
    }
  }
  else {
  ...
  if (deteccion_HSC || deteccion_MAG) {
...
void iniciar_HSCDTD008A() {
    segundoI2C.beginTransmission(HSCDTD008A_ADDRESS); // Set up the registers
    segundoI2C.write(0x1B);
    segundoI2C.write(0b10011000); //0x9A //98
    segundoI2C.endTransmission();
    segundoI2C.beginTransmission(HSCDTD008A_ADDRESS); 
    segundoI2C.write(0x1C);
    segundoI2C.write(0b00001100); //0C
    segundoI2C.endTransmission();
    segundoI2C.beginTransmission(HSCDTD008A_ADDRESS);
    segundoI2C.write(0x1E); 
    segundoI2C.write(0b10010000);//90
    segundoI2C.endTransmission();
}

void leer_HSCDTD008A(short &x, short &y, short &z) {
  segundoI2C.beginTransmission(HSCDTD008A_ADDRESS); 
  segundoI2C.write(0x10);
  segundoI2C.endTransmission();
  segundoI2C.requestFrom(HSCDTD008A_ADDRESS, 6);
  
  if (6 <= segundoI2C.available()) {
    x = segundoI2C.read(); x |= segundoI2C.read()<<8; 
