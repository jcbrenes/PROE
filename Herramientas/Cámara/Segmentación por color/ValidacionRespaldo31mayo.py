        ...
        for centro in range(len(centrosC1[imagen])):
            for centro2 in range(len(centrosC2[imagen])):
                ...
                if distance<=70:
                    ...
                    peer=(imagen, numColor1, numColor2, centro, centro2)
                    """if centro2==0:
...
def identificar_robots(Acyan, Amagenta, Aamarillo, Anaranja, centros_cCyan, centros_cMagenta, centros_cNaranja, centros_cAmarillo):
    ...
    """numImages=len(Acyan)
    robotsCyan=np.zeros((len(Acyan),len(Acyan[0])))        
    robotsMagenta=np.zeros((numImages,len(Amagenta[0])))
    robotsNaranja=np.zeros((numImages,len(Anaranja[0])))
    robotsAmarillo=np.zeros((numImages,len(Aamarillo[0])))
    #Inicia la detección de robots con cyan con círculo grande
    for lista in range(numImages):
        #contadores
        contC=0
        contM=0
        contN=0
        contA=0
        for area in Acyan[lista]:
            cont+=1
            #valida si es un área de círculo grande
            if area >=7000:
    """
                  
    return True
"""
def buscar_pareja(ListaCentros1, ListaCentros2, numImagen, numContorno, numColor):
    #Definición inicial, significa sin pareja asignada
    pareja=9
    
    #obtiene el centro del círculo grande
    posG=ListaCentros1[numImagen][numContorno]
    x1=posG[0]
    y1=posG[1]
    #Se busca en la misma imagen, en otro color si algún contorno está muy cerca,
    #esto es que sea la pareja de color del círculo grande, es decir, su
    #círculo pequeño
    for centro in ListaCentros2[numImagen]:
        #Obtiene las coordenadas
        x2=centro[0]
        y2=centro[1]
        #print("x1,x2,y1,y2", x1, x2, y1,y2)
        distance = math.sqrt( ((x1-x2)**2)+((y1-y2)**2) )
        #100 es el diámetro aproximado de un círculo grande
        if distance <=100:
            #osea está muy cerca, solo un círculo 
            #falta validar que el contorno 2 no sea un contorno grande
    return True
"""       
